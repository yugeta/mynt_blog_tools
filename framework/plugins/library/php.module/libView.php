...
class libView extends fw_define{
		...
		if(preg_match_all('@<'.$sp.'(.*?)'.$sp.'>@' , $tpl  , $fnc)){
			...
			for($i=0,$c=count($fnc[1]);$i<$c;$i++){
				...
				//テンプレートファイルの取得
					...
					$tpl = $this->check_query_val($tpl,$key,$val,$this->file2HTML($file),$sp);
					$replace_data = "";
					$tpl = $replace_data ? str_replace("<".$sp.$key.":".$val.$sp.">" , $replace_data , $tpl) : '';
				}
				//設定済みGlobal情報の取得
				else if($key=="globals"){
	...
	function check_query_val($tpl="",$key="",$val="",$replace_data=null,$sp="%"){
		if(!$replace_data){return;}
		// if(!$tpl || !$key || !$val){return;}
		// print_r([$tpl,$key,$val,$replace_data]); 
	...
	function check_class($tpl="",$data="",$sp="%"){
		...
		return $ret ? str_replace("<".$sp.join(":",$data).$sp.">" , $ret , $tpl) : '';
	}
}
